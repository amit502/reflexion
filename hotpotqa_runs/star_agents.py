...
#             for existing in self.knowledge[-20:]:   # check last 20 for speed
#                 sim = float(np.dot(q_emb, existing.embedding(self.embed_fn)))
#                 if sim > 0.92:
#                     return   # near-duplicate — discard
#         self.knowledge.append(knowledge)

#     def retrieve(self, action_intent: str, k: int = 2) -> List[StepKnowledge]:  # ← default k=2
#         if not self.knowledge:
#             return []
#         q_emb = self.embed_fn(action_intent)
#         d     = q_emb.shape[0]
...
#                  reflect_llm: AnyOpenAILLM = None,
#                  knowledge_store: StepKnowledgeStore = None,
#                  retrieval_k: int = 2,          # ← changed from 3 to 2
#                  use_reflection: bool = True):
...
#         # 5. Store knowledge — no extra LLM call
#         # ── CORRECTION FIX: inject prev_expected into scratchpad so LLM can compare ──
#         if correction and len(correction) > 15:
#             self.knowledge_store.add(StepKnowledge(
#                 action_intent = next_intent or action_str,
#                 rule          = correction,
...
#         # ── CORRECTION FIX: store expected so next step can see it in scratchpad ──
#         if expected:
#             self.scratchpad += f'\nPrevious Expected: {expected[:80]}'
...
# # ---------------------------------------------------------------------------
# # Two-shot examples in STAR format (replaces WEBTHINK_SIMPLE6)
# # Saves ~400 tokens vs 6 examples and teaches correct output format
# # ---------------------------------------------------------------------------

# # WEBTHINK_SIMPLE2_STAR = """Question: What is the elevation range for the area that the eastern sector of the Colorado orogeny extends into?
...
# # EXPECTED: The elevation range of the High Plains region in feet.
...
#         self.retrieval_k      = retrieval_k
#         self.use_reflection   = use_reflection
#         self.react_examples   = WEBTHINK_SIMPLE2   # ← 2 STAR-format examples
#         self.reflect_examples = REFLECTIONS
#         self.enc              = tiktoken.encoding_for_model("text-davinci-003")
#         self.reflections: List[str] = []
#         self.reflections_str  = ''
...
class STARReactAgent:
    ...
    Mismatch injection: when previous EXPECTED != actual observation,
    a NOTE is added to the prompt so LLM writes CORRECTION alongside next action.
    ...
    def step(self) -> None:
        ...
        # ── Store prev expected/observation for mismatch injection ────────────
    ...
    def __reset_agent(self) -> None:
        ...
        self.scratchpad            = ''
        self._prefetched_knowledge = []
        self._prev_expected        = ''   # ← new
        self._prev_observation     = ''   # ← new
