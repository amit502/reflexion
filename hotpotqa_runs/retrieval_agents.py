...
class ReflexionStrategy(Enum):
    ...
    LAST_ATTEMPT_AND_SUMMARIZED_REFLEXION: Summarize all reflections into one compressed belief state
    RETRIEVED_TRAJECTORY_REFLEXION: Quantum density matrix retrieval — represents each trajectory
                                    as a mixed-state density matrix over semantic aspects, uses
                                    Von Neumann trace overlap for similarity.
    """
    NONE = 'base'
    LAST_ATTEMPT = 'last_trial'
    REFLEXION = 'reflexion'
...
def _build_density_matrix(vecs: List[np.ndarray],
    ...
    """
    Build a density matrix ρ = Σ_k w_k |φ_k><φ_k| from a list of
    unit vectors and their weights.

    Each vector φ_k represents one semantic aspect of the trajectory:
        φ_0 = question embedding       (what the task is about)
        φ_1 = error/reflection embedding (how it failed)
        φ_2 = scratchpad embedding    (what actions were taken)

    The density matrix is a superposition of these aspects, capturing
    that a trajectory simultaneously belongs to multiple semantic spaces.

    Args:
        vecs:    list of unit vectors, shape [(d,), ...]
        weights: list of non-negative weights summing to 1

    Returns:
        ρ: (d, d) density matrix, Hermitian, trace = 1
    """
    assert len(vecs) == len(weights), "vecs and weights must have same length"
    d = vecs[0].shape[0]
...
def _quantum_similarity(rho_q: np.ndarray, rho_i: np.ndarray) -> float:
    """
    Quantum similarity via Von Neumann trace overlap:
        sim(ρ_q, ρ_i) = Tr(ρ_q · ρ_i)

    This is the quantum analog of cosine similarity — it measures the
    overlap between two mixed states. For pure states (rank-1 matrices),
    it reduces exactly to squared cosine similarity: Tr(|q><q| · |i><i|) = <q|i>^2.

    For mixed states it additionally captures uncertainty/spread across
    multiple semantic aspects, making it more robust than single-vector cosine.
...
# ---------------------------------------------------------------------------
# Trajectory store — quantum density matrix episodic memory
# ---------------------------------------------------------------------------
...
class TrajectoryRecord:
    ...
    Extended with quantum density matrix representation:
    each trajectory is represented as a mixed state over 3 semantic aspects:
        - question semantics  (what was being asked)
        - failure mode        (how it failed — error class + reflection)
        - action pattern      (what actions were taken — scratchpad)
    """
    ...
    def __init__(self,
        ...
        # Cached embeddings per aspect
        self._q_emb:   Optional[np.ndarray] = None   # question embedding
        self._err_emb: Optional[np.ndarray] = None   # error/reflection embedding
        self._act_emb: Optional[np.ndarray] = None   # action/scratchpad embedding
        # Cached density matrix
        self._density_matrix: Optional[np.ndarray] = None

    def _get_error_text(self) -> str:
        """Text representing the failure mode aspect."""
        parts = []
        if self.error_class and self.error_class != "UNKNOWN":
    ...
    def _get_action_text(self) -> str:
        """Text representing the action pattern aspect — first 300 chars of scratchpad."""
        return self.scratchpad[:300] if self.scratchpad else ""

    def density_matrix(self, embed_fn,
                       w_question: float = 0.5,
                       w_error: float = 0.3,
                       w_action: float = 0.2) -> np.ndarray:
        """
        Returns the density matrix ρ for this trajectory.

        ρ = w_q |q><q| + w_e |e><e| + w_a |a><a|

        where:
            |q> = question embedding     (what task)
            |e> = error+reflection emb   (failure mode)
            |a> = action pattern emb     (how agent behaved)
        ...
        if self._q_emb is None:
            self._q_emb = embed_fn(self.question)
        if self._err_emb is None:
            self._err_emb = embed_fn(self._get_error_text())
        if self._act_emb is None:
            self._act_emb = embed_fn(self._get_action_text())

        self._density_matrix = _build_density_matrix(
            vecs=[self._q_emb, self._err_emb, self._act_emb],
            weights=[w_question, w_error, w_action]
        )
        return self._density_matrix

    # Keep backward-compatible embedding() for any code that calls it
    def embedding(self, embed_fn) -> np.ndarray:
        if self._q_emb is None:
            self._q_emb = embed_fn(self.question)
        return self._q_emb
...
class TrajectoryStore:
    """
    Episodic memory of past trajectories with Quantum Density Matrix retrieval.

    Retrieval uses Von Neumann trace overlap between density matrices:
        sim(ρ_query, ρ_i) = Tr(ρ_query · ρ_i)

    Each trajectory is represented as a mixed state over 3 semantic aspects:
        ρ = w_q |question><question| + w_e |error><error| + w_a |actions><actions|

    This captures that a trajectory simultaneously encodes what was asked,
    how it failed, and what actions were taken — rather than collapsing all
    information into a single vector.

    MMR diversity is also computed via quantum similarity (trace overlap)
    rather than cosine similarity, maintaining theoretical consistency.
    """

    def __init__(self,
                 embed_fn=None,
                 # Density matrix aspect weights
                 w_question: float = 0.5,
                 w_error:    float = 0.3,
                 w_action:   float = 0.2,
                 # MMR trade-off
                 mmr_lambda: float = 0.5,
                 # Reflection quality filter
                 min_reflection_length: int = 50):
        """
        Args:
            embed_fn:               callable(text) -> unit np.ndarray
            w_question/error/action: density matrix aspect weights (must sum to 1)
            mmr_lambda:             relevance vs diversity trade-off in MMR
            min_reflection_length:  minimum reflection length to store (quality filter)
        """
        assert abs(w_question + w_error + w_action - 1.0) < 1e-6, \
    ...
    def add(self, record: TrajectoryRecord) -> None:
        """
        Add a trajectory to the store with quality filtering.
        Successes are always stored.
        Failures are only stored if reflection is meaningful.
        """
        if record.success:
            self.records.append(record)
    ...
    def retrieve(self,
                 ...
                 max_successes: int = 2) -> List[TrajectoryRecord]:
        """
        Retrieve top-k trajectories using quantum density matrix similarity.

        1. Build query density matrix from (question, error_class, scratchpad)
        2. Score all stored trajectories via Tr(ρ_query · ρ_i)
        3. Split into failure/success pools
        4. Apply MMR (using quantum similarity) within each pool
        5. Return balanced failure/success set
        """
        if not self.records:
            return []
        ...
        rho_query = self._build_query_density_matrix(
            question, error_class, scratchpad
        )

        # Score every record via quantum trace overlap
        scored: List[Tuple[float, TrajectoryRecord]] = []
        for rec in self.records:
            rho_i = rec.density_matrix(
                self.embed_fn,
                w_question=self.w_question,
        ...
        # Split into failure / success pools
        failures  = [(s, r) for s, r in scored if not r.success]
        successes = [(s, r) for s, r in scored if r.success]

        # MMR selection within each pool using quantum similarity
        selected_failures  = self._mmr_select(failures,  max_failures)
        selected_successes = self._mmr_select(successes, max_successes)
    ...
    def _build_query_density_matrix(self,
                                     question: str,
                                     error_class: str,
                                     scratchpad: str) -> np.ndarray:
        """Build the query density matrix from current context."""
        error_text = error_class if error_class != "UNKNOWN" else question
        action_text = scratchpad[:300] if scratchpad else question

        q_emb   = self.embed_fn(question)
        err_emb = self.embed_fn(error_text)
        act_emb = self.embed_fn(action_text)

        return _build_density_matrix(
            vecs=[q_emb, err_emb, act_emb],
            weights=[self.w_question, self.w_error, self.w_action]
        )
    ...
    def _mmr_select(self,
        ...
        Greedy MMR using quantum similarity for diversity:
            next = argmax [ λ * Tr(ρ_q·ρ_i) - (1-λ) * max_j∈S Tr(ρ_i·ρ_j) ]
        """
        if not scored:
            return []
        ...
        while len(selected) < k and candidates:
            if not selected:
                ...
                best = None
                sel_matrices = [
                    r.density_matrix(self.embed_fn,
                                     self.w_question, self.w_error, self.w_action)
                    for r in selected
                ]
                for rel_score, rec in candidates:
                    rho_i = rec.density_matrix(
                        self.embed_fn, self.w_question, self.w_error, self.w_action
                    )
                    # Max quantum similarity to already-selected trajectories
                    max_sim_to_selected = max(
    ...
    # ------------------------------------------------------------------
    # Sentence transformer embedding (unchanged)
    # ------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ReactReflectAgent — quantum density matrix retrieval
# ---------------------------------------------------------------------------
...
class ReactReflectAgent(ReactAgent):
    ...
    def _reflect_with_retrieval(self) -> None:
        """
        Quantum Density Matrix Retrieval:

        1. Classify failure into error taxonomy.
        2. Build query density matrix from (question, error_class, scratchpad).
        3. Score stored trajectories via Von Neumann trace overlap Tr(ρ_q · ρ_i).
        4. Apply MMR with quantum similarity for diversity.
        5. Generate contrastive reflection.
        6. Store trajectory (with quality filter).
        """
        ...
        # Step 2-4 — quantum retrieval (scratchpad passed for action aspect)
        retrieved = self.trajectory_store.retrieve(
            question=self.question,
            error_class=self._current_error_class,
            scratchpad=self.scratchpad,          # ← new: action pattern aspect
            k=self.retrieval_k,
            max_failures=self.retrieval_max_failures,
            max_successes=self.retrieval_max_successes,
        )
        ...
        current_reflection = format_step(
            self.reflect_llm(reflection_prompt, REFLECTION_SYSTEM_PROMPT)
        )
        self.reflections += [current_reflection]

        # Assemble reflections_str
        self.reflections_str = format_last_attempt(self.question, self.scratchpad)
        self.reflections_str += format_reflections(
            self.reflections, header=REFLECTION_AFTER_LAST_TRIAL_HEADER
        )

        # Step 7 — store with quality filter (handled inside TrajectoryStore.add)
        self.trajectory_store.add(TrajectoryRecord(
            question=self.question,
            scratchpad=self.scratchpad,
            reflection=current_reflection,
